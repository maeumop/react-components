...
import { useValidation } from '../hooks.ts';
      ...
      (evt: KeyboardEvent): void => {
        ...
        if (code === 'arrowdown') {
          setSelectedKeyIndex(prev => {
            selectedIndex = prev >= len - 1 ? 0 : prev + 1;

            // 다음 렌더링 사이클에서 스크롤 실행
            setTimeout(() => {
              const targetElement = li[selectedIndex];
          ...
          setSelectedKeyIndex(prev => {
            selectedIndex = prev <= 0 ? len - 1 : prev - 1;
          ...
          if (multiple && !searchable && selectedKeyIndex === 0) {
            ...
            if (selectedIndex > -1 && selectedIndex < optionList.length) {
              const { value } = optionList[selectedIndex];
              selectOption(value, selectedIndex);
            }
          }
        }
      },
